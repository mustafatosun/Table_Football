...
namespace Table_Football
{
    public partial class GamePage2 : PhoneApplicationPage
    {
        ...
        Vector2 ilk1 = Vector2.Zero;
        Vector2 son1 = Vector2.Zero;
        ...
        private void OnUpdate(object sender, GameTimerEventArgs e)
        {
            ...
            if (top==0)
            {
                ...
                {
                    Settings.result = 2;
                    NavigationService.Navigate(new Uri("/WinLose2.xaml", UriKind.Relative));
                }
                else
                {
                    Settings.result = 1;
            ...
            //        for (int i = 0; i < 10; i++)
            //        {
            //            team2[i].position.X += (int)gesture.Delta.X;
            ...
            //team1[0].update(ball, team1);
            //team2[0].update(ball, team2);
            ...
            foreach (TouchLocation touch in touches)
            {
                ...
                else
                {
                    if (touch.State == TouchLocationState.Moved)
                    {
                        ...
                        int farkX = (int)(son1.X - ilk1.X);
                        for (int i = 0; i < 10; i++)
                        {
                            team2[i].position.X += (int)farkX;
                        }
                        if (team2[0].position.X < 150 || team2[0].position.X > 300)
                        {
                            for (int i = 0; i < 10; i++)
            ...
            TouchCollection touches1 = TouchPanel.GetState();
            foreach (TouchLocation touch in touches1)
            {
                if (touch.Position.Y > 424)
                {
                    if (touch.State == TouchLocationState.Moved)
